...
function ToDoWrapper({ todos, head, toggleComplete, deleteTodo, editTodo }: any) {
  ...
  function isDueToday(value: { taskDate: string | number | Date; completed: boolean; }) {
    ...
    if ((newTaskDate.toDateString() == new Date(new Date().toDateString()).toDateString()) && (value.completed === false)) {
      return value;
    }
  }
